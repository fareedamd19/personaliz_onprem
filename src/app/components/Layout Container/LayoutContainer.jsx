  ...
  return (
    ...
    >
      ...
      >
        ...
        {currentQuestionData?.current?.type !== "video" && (
          <div
            style={{
              height: getHeightOfQuestionContainer(),
              transition: isQuestionOnTopOfVideo ? "height 0.2s" : "",
            }}
            className={`${
              styles.layoutOuterMostScrollableCont
            } overflow-y-auto ${
              isQuestionOnTopOfVideo ? "w-full" : "w-[50%]"
            } ${
              isQuestionOnTopOfVideo
                ? `absolute bottom-0 left-0 z-10`
                : "h-full"
            } ${isQuestionOnTopOfVideo && "bg-black bg-opacity-20"}`}
          >
            {isQuestionOnTopOfVideo && (
              <div
                style={getPositionOfUpAndDownArrow()}
                className="w-max h-max flex flex-col gap-2 absolute top-0 right-2 z-50"
              >
                {questionContainerHeight !== "top" && (
                  <span
                    onClick={() => handleQuestionConatinerUpOrDown("up")}
                    className="bg-white p-0 rounded cursor-pointer"
                  >
                ...
                {questionContainerHeight !== "bottom" && (
                  <span
                    onClick={() => handleQuestionConatinerUpOrDown("down")}
                    className="bg-white p-0 rounded cursor-pointer"
                  >
