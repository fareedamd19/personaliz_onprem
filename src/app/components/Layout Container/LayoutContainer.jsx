...
const LayoutContainer = () => {
  const {isQuestionOnTopOfVideo,questionContainerHeight,handleQuestionConatinerUpOrDown,getHeightOfQuestionContainer,getPositionOfUpAndDownArrow}=useGlobalStoreContext()
...
<div style={{height:getHeightOfQuestionContainer()}} className={`${styles.layoutOuterMostScrollableCont} overflow-y-auto ${isQuestionOnTopOfVideo?'w-full':'w-[50%]'} ${isQuestionOnTopOfVideo?`absolute bottom-0 left-0 z-10`:'h-full'} ${isQuestionOnTopOfVideo&&'bg-black bg-opacity-20'}`}>

{isQuestionOnTopOfVideo&&<div style={getPositionOfUpAndDownArrow()} className='w-max h-max flex flex-col gap-2 absolute top-0 right-2'>
