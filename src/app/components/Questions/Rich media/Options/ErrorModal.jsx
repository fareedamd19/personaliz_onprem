...
const ErrorModal = ({handleGoBack}) => {
  return (
   <>
      <section className='w-full h-full flex flex-col items-center justify-center p-4 bg-white gap-7'>
        <h1 className='text-black text-xl font-bold text-center'>Please enable camera & microphone permissions</h1>
        ...
        {window.innerWidth>600?<p>{`Click the 🔒lock icon in your browser's address bar, set the correct permissions and then refresh this page.`}</p>:<>
            <ol className='flex flex-col gap-2'>
                ...
                <li className='text-sm'><span className='text-red-500 mr-2'>2.</span>Make sure to set the correct camera & microphone permissions for current website.</li>
