...
const FileSelectedModal = ({ filesArray, handleRemoveFile, handleJump }) => {
  const { is_RTL, fontThemeObj, optionThemeObj, globalHardcodedVariables } =
  ...
  return (
    <>
      <section className="w-full flex flex-col gap-5">
        <div
          className={`${styles.scrollableContainer} w-full flex flex-col gap-3 max-h-[50dvh] md:max-h-[60dvh] overflow-y-auto`}
        >
          {filesArray.map((file) => {
            return (
              <div key={file.id} className="flex items-center gap-4 w-full">
                <p
                  className="border-2 border-dashed backdrop-blur rounded-md px-3 py-2 w-[80%] md:w-full break-words font-semibold"
                  style={{
                ...
                <p
                  ...
                  className="border border-dashed backdrop-blur rounded-md w-6 h-8 flex cursor-pointer"
                  style={{
                    borderColor: optionThemeObj?.option_text_color,
                    backgroundColor: addOpacity(
                      optionThemeObj?.option_text_color
                ...
                >
                  <RiDeleteBin6Line className="m-auto text-lg" />
                </p>
              </div>
            );
          })}
        ...
        <button
          onClick={handleJump}
          className="py-1 px-4 font-medium w-max m-auto"
          style={{
        ...
        >
          {globalHardcodedVariables?.current?.submitText}
        </button>
      </section>
    </>
  );
