...
const CountryCodePicker = ({choosenCountryCode,selectedCountry,setCountrySelected,optionThemeObj}) => {
   ...
   const[loading,setLoading]=useState(true)
    ...
    const DropdownNode = useClickOutside(() => setIsCountryCodePickerClick(false), ignoresDropDownNode.current)
...
if(choosenCountryCode&&countryCodeArr.length>0){
  const countryAccrodingToIp=countryCodeArr.filter((cc)=>{return cc.isoCode===choosenCountryCode})
...
//eslint-disable-next-line
 ...
 <>
    ...
    {selectedCountry&&<Image src={selectedCountry?.flag} alt='map' width={30} height={25}/>}
    <RiArrowDownSLine style={{marginLeft:".25rem",fontSize:"1.1rem",color:optionThemeObj?.option_text_color}}/>
    </div>
    {isCountryCodePickerClick&&<div ref={DropdownNode} className={styles.signup_form_onboarding_countryCode_selector_inputCont_selector}>
    ...
    return <>
    <div style={{background:selectedCountry.isoCode===cc.isoCode?"#476A7E":"",color:selectedCountry.isoCode===cc.isoCode?"#fff":""}} onClick={()=>{setIsCountryCodePickerClick(false);setCountrySelected(cc)}} key={cc.isoCode}  className={styles.signup_form_onboarding_countryCode_selector_inputCont_selector_option}>
